...
#define MAX_PLAYLIST_COUNT 1000
...
int main(int argc, char *argv[])
{
   ...
   std::string           m_filenames [ MAX_PLAYLIST_COUNT ];
   double                m_incr                = 0;
   CRBP                  g_RBP;
   COMXCore              g_OMX;
   bool                  m_stats               = false;
   ...
   const int sync_server_opt = 0x208;
   const int sync_port_opt = 0x209;
   const int sync_client_opt = 0x20a;
   const int server_address_opt = 0x20b;
   const int sync_num_clients_opt = 0x20c;
   const int sync_verbose_opt = 0x20d;
  ...
  m_filename = argv[optind];
   ...
   // New functionality to allow for playlists of files
   for ( i = 0; i < MAX_PLAYLIST_COUNT && i + optind < argc; i++)
   {
      m_filenames [ i ] = argv [ i + optind ];
   }
  ...
  // THE BEES KNEES RIGHT HERE FOLKS!!!!!
  while(!m_stop)
   {
      if(g_abort)
      goto do_exit;
      ...
      /* The way things are set up at the moment, this coding is a    *
       * total mess. For example, there are no states associated with * 
       * trasnitioning in and out of:                                 * 
       *   -Fast Forwarding                                           * 
       *   -Rewinding                                                 * 
       *   -Pausing                                                   * 
       *   -...                                                       * 
       * Therefore, it's hard to predict if the clients are out of    * 
       * sync, or if one just started fast forwarding more quickly.   * 
       * For this reason, and others, ALL functionality will be       * 
       * supressed, and must be set from the command line             */
      if (update) {
